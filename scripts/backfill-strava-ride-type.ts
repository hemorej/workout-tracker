/**
 * One-off backfill: populate workouts.ride_type ('outdoor' | 'trainer') for
 * historical entries logged before this field existed, using the matching
 * ride's Strava activity type ('Ride' vs 'VirtualRide').
 *
 ...
 * with ride_type IS NULL are touched — safe to re-run.
 *
 * Usage:
 *   pnpm dlx dotenv-cli -e .env -- node --experimental-strip-types scripts/backfill-strava-ride-type.ts
 */
...
async function main() {
  ...
  const { rows: missing } = await pool.query<{ id: number, date: string, name: string }>(
    `SELECT id, date::text AS date, name FROM workouts
     WHERE user_id = $1 AND ride_type IS NULL
     ORDER BY date ASC`,
    [userId],
  )

  if (missing.length === 0) {
    console.log('No workouts are missing ride_type — nothing to backfill.')
    await pool.end()
    return
  }

  const earliestDate = missing[0]!.date
  console.log(`Backfilling ride_type for ${missing.length} workout(s) from ${earliestDate} onward…`)
  ...
  for (const workout of missing) {
    ...
    const rideType = ride.type === 'VirtualRide' ? 'trainer' : 'outdoor'
    await pool.query('UPDATE workouts SET ride_type = $1 WHERE id = $2', [rideType, workout.id])
    matched += 1
  }
