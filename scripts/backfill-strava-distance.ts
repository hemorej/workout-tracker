/**
 * One-off backfill: populate workouts.distance_km for historical entries
 * that were logged before the distance field existed, using the matching
 * ride's distance from the Strava API.
 *
 ...
 * with distance_km IS NULL are touched — safe to re-run.
 *
 * Usage:
 *   pnpm dlx dotenv-cli -e .env -- node --experimental-strip-types scripts/backfill-strava-distance.ts
 */
...
async function main() {
  ...
  const { rows: missing } = await pool.query<{ id: number, date: string, name: string }>(
    `SELECT id, date::text AS date, name FROM workouts
     WHERE user_id = $1 AND distance_km IS NULL
     ORDER BY date ASC`,
    [userId],
  )

  if (missing.length === 0) {
    console.log('No workouts are missing distance_km — nothing to backfill.')
    await pool.end()
    return
  }

  const earliestDate = missing[0]!.date
  console.log(`Backfilling distance for ${missing.length} workout(s) from ${earliestDate} onward…`)
  ...
  for (const workout of missing) {
    ...
    const distanceKm = Math.round((ride.distance / 1000) * 10) / 10
    await pool.query('UPDATE workouts SET distance_km = $1 WHERE id = $2', [distanceKm, workout.id])
    matched += 1
  }
